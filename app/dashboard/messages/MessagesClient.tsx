...
type Filter = "active" | "unread" | "read" | "new" | "archived";
...
export default function MessagesClient({ canDeleteMessages }: { canDeleteMessages: boolean }) {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conversation) => {
      const archived = conversation.messages.every((message) => message.archived);
  ...
  const activeConversations = conversations.filter((conversation) => !conversation.messages.every((message) => message.archived));
  const archivedConversations = conversations.filter((conversation) => conversation.messages.every((message) => message.archived));
  const read = messages.filter((message) => !message.archived).length - unread;
  const newNumbers = activeConversations.filter((conversation) => !conversation.patientId).length;
  ...
  async function setConversationArchived(phone: string, archived: boolean) {
    const res = await fetch("/api/messages", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ phone, action: archived ? "archive" : "unarchive" }),
    });
    if (res.ok) {
      setMessages((prev) => prev.map((message) => (
        message.phone === phone ? { ...message, archived, read: true } : message
      )));
      setSelectedPhone(null);
      setToast({ ok: true, text: archived ? "تمت أرشفة المحادثة" : "تمت إعادة المحادثة للصندوق" });
    }
  }

  async function deleteConversation(phone: string) {
    if (!window.confirm("حذف المحادثة نهائياً؟")) return;
    const res = await fetch("/api/messages", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ phone }),
    ...
    if (res.ok) {
      setMessages((prev) => prev.filter((message) => message.phone !== phone));
      setSelectedPhone(null);
      setToast({ ok: true, text: "تم حذف المحادثة نهائياً" });
    } else {
      setToast({ ok: false, text: data.error ?? "تعذر حذف المحادثة" });
    }
  }
  ...
  const filterTabs: { value: Filter; label: string; count: number }[] = [
    { value: "active", label: "الصندوق", count: activeConversations.length },
    { value: "unread", label: "غير مقروء", count: activeConversations.filter((conversation) => conversation.unread > 0).length },
    { value: "read", label: "تمت مراجعتها", count: activeConversations.filter((conversation) => conversation.unread === 0).length },
    { value: "new", label: "أرقام جديدة", count: newNumbers },
    { value: "archived", label: "المؤرشفة", count: archivedConversations.length },
  ];
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {selectedConversation.messages.every((message) => message.archived) ? (
                          ...
                          <button onClick={() => setConversationArchived(selectedConversation.phone, true)} className="rounded-2xl bg-slate-100 px-4 py-2.5 text-xs font-black text-slate-700 ring-1 ring-slate-200 transition hover:bg-slate-200">
                            أرشفة
                          </button>
                        )}
                        {canDeleteMessages && (
                          <button onClick={() => deleteConversation(selectedConversation.phone)} className="rounded-2xl bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-700 ring-1 ring-rose-100 transition hover:bg-rose-100">
                            حذف نهائي
                          </button>
                        )}
                      </div>
