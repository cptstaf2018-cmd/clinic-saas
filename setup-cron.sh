...
# تذكيرات المرضى — كل 5 دقائق حتى تصل قبل الموعد بدقة
*/5 * * * * curl -s -o /dev/null -w "\%{http_code}" -H "Authorization: Bearer ${CRON_SECRET}" ${APP_URL}/api/cron/reminders >> /var/log/clinic-cron.log 2>&1
